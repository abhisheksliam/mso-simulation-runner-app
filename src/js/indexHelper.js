...
$('.sidebar-menu').on('click', '.add-action', function(e) {
    ...
    el.remove();
    actionTree.append('<li data-tree=\'{"item":"'+(parseInt(clickedAddActionNodeDataTree.item))+'","method":"'+(parseInt(clickedAddActionNodeDataTree.method))+'","action":"'+(parseInt(clickedAddActionNodeDataTree.action) + 1)+'"}\' class="add-action"><a href="#" class="copy-action"><i class="fa fa-copy text-lime"></i> <span>New / Duplicate Action</span></a></li>');
...
$('.sidebar-menu').on('click', '.item-node', function(event) {
	if($(event.target).closest('li').hasClass( "item-node" )) {
		...
		$('.delete-method-node').hide();
    	$('.reorder-up, .reorder-down').hide();
		$('.item-node').eq($(this).data('tree').item - 1).find('.action-tree').hide();
		$('.method-details-section').hide();
        $('.action-details-section').hide();
	    $('.baloo-description').hide();
	}
...
$('.sidebar-menu').on('click', '.delete-method-node', function(e) {
	...
	$('.item-node').eq(_currentAddMethodData.item -1).find('.method-node').eq(currentMethodNumber - 1).find('.delete-method-node').show();
	$('.item-node').eq(_currentAddMethodData.item -1).find('.method-node').eq(currentMethodNumber - 1).find('.action-tree').show();
...
$('.sidebar-menu').on('click', '.duplicate-method', function(e) {
    ...
    if(taskData.items[parseInt(clickedMethodDataTree.item)-1].methods.length >= parseInt(clickedMethodDataTree.method)){
		...
		$('.item-node').eq(currentItemNumber - 1).find('.method-node').eq(clickedMethodDataTree.method).find('.delete-method-node').show();
